import { useStringifySearchParam } from '../src';

describe('returns a search string from regular key/value object', () => {
  test('with single param', () => {
    expect(useStringifySearchParam({ topic: 'api' })).toBe('topic=api');
  });
  test('with multiple params', () => {
    expect(useStringifySearchParam({ topic: 'api', something: 'else' })).toBe(
...
describe('returns a search string from object that contains array', () => {
  test('with default config', () => {
    expect(
      useStringifySearchParam({ topic: ['api', 'new-api', 'another-api'] })
